...
10. `audit`: persist compact evidence, prompt/model version, and policy version.
...
gate_policy -> audit
gate_policy -> cv_detector -> ocr_detector -> dialogue_detector -> vision_reasoner -> fusion -> policy -> audit
```

Skip `vision_reasoner` when Aliyun scoped decision is `PASS`. For failed videos, run VLM refinement before finalizing the report so the evidence stays targeted.
...
Return strict JSON with categories, scores, risk_level, confidence, reason, and evidence.
...
- Use `scripts/run_aliyun_video_moderation.py` for provider-backed review reports and normalization before integrating the same contracts into Celery.
