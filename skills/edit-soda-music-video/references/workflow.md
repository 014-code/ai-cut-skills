...
- BGM 路径及其选择来源、素材根目录、logo 版本、尾帧文件；
- 是否金币、是否歌单、歌曲禁投审查结果；
- 金额档位、允许的利益点；
- 口播字幕文本和每个物料的入点/出点。
- 若调用方提供口播文件，确认其为字幕文本的权威来源。先运行 `caption-budget`，根据当前画布和字幕样式只计算一次本任务单行字数上限；执行模型复制原台词并只插入语义换行。字幕时序仍必须来自对当前输入执行的 Whisper `--word_timestamps True`。移除普通标点，但保留数字内部小数点。

用户明确指定渠道或 BGM 时直接采用。用户未指定渠道时，执行模型必须自行选择：先读取口播、金额、金币/提现、歌单和素材画面线索，再从 `old-down`、`new-high-mid`、`free-listen`、`coin-non-down` 中选择能完整约束当前内容的正式渠道；多个渠道均可时选择文案和金额限制更严格且与素材最一致的一项，不使用 `general` 交付正式成片。用户未指定 BGM 时，执行模型必须自行选择：从任务目录或调用方提供的真实音频候选中检查路径、可解码性、投放范围、节奏、歌词/人声干扰、强拍突变和循环适配性，直接选出最适合口播的一首；视觉 Manifest 不保存音频记录。选择过程不向用户追问渠道或 BGM，并在计划、合规报告或交付说明中记录最终值和理由；只有没有任何可用 BGM 文件时才报告缺少必需素材。时长只记录实际结果，不作为渲染门槛。
...
先逐句识别利益点。只有明确表达产品功能带来的用户价值、合规权益、已授权优惠或具体使用收益的台词才允许选材；寒暄、过渡、情绪铺垫、泛泛描述、单独 CTA 和免责声明不查询、不匹配、不插入普通素材。对每条利益点先按 [special-material-matches.md](special-material-matches.md) 判断特殊规则；命中时优先使用参考素材或符合画面条件的轻度变体，并写入 `special_match_rule`；未命中才把利益点文本交给通用 Skill，读取包含候选和理由的 `asset_candidates.json`。汽水层结合合规、布局和素材连续性决定是否采用，不能自动采用候选第一名。所有最终素材都要写入真实路径、`semantic_role=benefit_point` 和 `matched_benefit_text`。完整种类和输入要求见 [asset-requirements.md](asset-requirements.md)。如果传入口播文件，先运行 `caption-budget --timeline-json ...`，读取一次任务级 `max_characters_per_line`。执行模型按该上限为整份台词插入语义换行：优先在动作与结果、条件与结论、转折、因果或并列意群之间切分，不按字数平均切分，不改写原词。再对去气口后的当前输入运行 `repair-captions --input ...`；每个语义行映射为一个 Whisper input caption，修正版时间轴会改为 `time_mode=input`。执行模型随后必须直接读取修正字幕 input `start/end`，重算所有素材，再进入 `render`。
...
填写字体时必须使用字体文件的真实内部 family name，不能把文件名或正文字体名称复制给 SodaFont。字幕样式写入 `font.caption_style`；模板已把字号 13、缩放 100%、黑色细描边、阴影 0、`x=0,y=-500` 转为 1080×1920 ASS renderer 的校准配置：`outline=3`、`shadow=0`。任务开始时根据当前画布、字号、横向缩放、字间距、描边和安全边距动态计算一次单行字数上限，后续全部台词复用，不在文档或模板中写死。语义分段后的字幕事件仍由渲染器按实际像素宽度显式均衡换行，优先一到两行、最多三行，不依赖 ASS 自动换行。不得拆分“汽水音乐”、小数金额或英文单词；三行仍无法容纳时，回到模型语义分段阶段，不得缩小字号或由脚本平均拆句。字幕文本去除普通中英文标点，但保留数字内部小数点；不得添加字幕背景条或半透明承托层。

素材无需避让字幕安全区。1080×1920 默认品牌保护边界对应素材可用区 `x=48..1032, y=320..1740`，但碰撞判断只使用 Manifest 的 effective_region 映射结果。透明留白、纯色空白边距和无内容画布可以越过保护边界，不触发变换。`phone`、`full_alpha` 和 `cta_icon` 默认保持完整源文件的源像素尺寸与原始比例；`icon` 在渲染前裁切 effective_region 并保持有效内容源像素尺寸。未显式填写 `y` 时，渲染器根据该素材时段内的实际字幕块动态计算纵坐标：图标底边放在字幕上边界上方 `72px`，字幕换行时同步上移；显式 `y` 始终优先。这一计算只改坐标，不改变尺寸，并在静态叠加和动效中共用同一解析后坐标。不得对任何布局做 `650×1050`、`300×300`、230px 宽或整画布拉伸等默认标准化。只有人物、图标、文字、UI 等有效内容真实碰撞时才先移动，移动无法解决时再从源尺寸等比缩小到最大安全比例。Remotion 沿用相同策略：effective_region 只用于碰撞判断，只有 icon 裁切 effective_region，其他布局的动效保留完整源文件。
