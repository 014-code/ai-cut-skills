...
- `caption-budget`：读取时间轴中的实际画布与 `font.caption_style`，只计算一次本任务 `caption_character_budget.max_characters_per_line`；数值不写入模板，字号、缩放、字间距、描边或安全边距变化时自动重算；
- `captions`：字幕入点、出点和文本；渲染时去除普通中英文标点，但保留并规范化数字内部小数点；执行模型必须先按动态字数上限插入语义换行，一行对应一个 caption event；
- `--script-file`：调用方提供口播文本时，作为字幕文案最高优先级；传入 `render`/`repair-captions` 前由执行模型复制原文并只插入语义换行。随后必须对当前输入执行 Whisper `--word_timestamps True`，把每行台词填入真实词级时间戳后生成修正版时间轴，修正版顶层固定为 `time_mode=input`，不覆盖原始时间轴；
...
- `font/logo/tail`：字体、黑白 logo 和尾帧路径；`font.body_family` 和 `font.brand_family` 必须是单一真实内部 family 名称，不接受逗号分隔 fallback 或 ASS 控制字符；`font.body_color` 默认白色，`font.brand_color` 默认品牌绿 `#3BFD42`；
...
`caption-budget`、`preflight` 和 `render` 使用同一字幕布局器。预算报告记录一次任务级 `available_width`、参考中文字符宽度和动态 `max_characters_per_line`；字幕修复报告记录模型语义行、各行字符数与 `automatic_width_split_count=0`。`preflight` 的 `caption_layout` 继续记录标准化文本、最终分行、`line_count`、每行估算宽度和 `available_width`；超过三行或任一行越过安全宽度时直接失败。最终 QA 仍需抽取最长字幕帧复核真实字体渲染结果。
