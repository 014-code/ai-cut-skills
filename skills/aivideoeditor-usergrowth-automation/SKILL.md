...
description: "Standalone UserGrowth automation skill for AIVideoEditor desktop upload workflows. Use when Codex needs to select specific videos or run multi-batch concurrent UserGrowth 自动上传 by skill scripts only, including dry-run planning, batch manifests/concurrency, song-library matching, 回填 Excel, CID 后歌曲名称列, Playwright upload/录入变色龙/送审/CID 回填, 素材分类标签/自定义标签, ddddocr 验证码, task.json/run.log/debug snapshots, or debugging the copied automation implementation."
---
...
The CLI supports `--video`, `--video-glob`, `--video-list`, `--all-videos`, `--split-by-song`, single-run JSON manifests, and multi-batch manifests with top-level `batches` plus `--concurrency`. It fails when a requested selector does not match any video.
