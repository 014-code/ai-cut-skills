...
7. Upload files and enter 录入变色龙 with retry. After the delivery product/platform modal closes, keep the page open until every material card on the current page has rendered actionable `UGC内容`, `分类标签`, and `自定义标签` controls; Redfruit also requires `创意源`. Do not start tagging while only card titles or partial forms are visible. Opening a classification selector is another wait gate for both Soda and Redfruit: when the current classification modal shows `暂无数据`, `暂无内容`, `暂无分类`, `暂无标签`, or another short `暂无...` empty state, cancel only that modal, wait with exponential backoff, reopen it, and continue until the required fields exist.
...
Redfruit orders are checkpointed independently of the browser session. The browser records the current order stage, upload task ID, review task ID, current ARLP task ID, `arlp_stage_index`, completed `arlp_stage_task_ids`, classification task ID, and per-material CID/status before continuing. It also records `arlp_stage_progress`, one entry per ARLP configuration, including `selection_started`, `modal_open`, `submitting`, `task_created`, `waiting_result`, `partial_failure`, or `success`, the attempt number, task ID, total/success/failure counts, and the last error. Classification writes `classification_progress` after each field, including field index/name/path, fill attempt, save status, and last error. A browser or network interruption can therefore resume from the exact ARLP task wait or unfinished classification field without re-uploading files or recreating a submitted ARLP task. A task ID already listed in `arlp_stage_task_ids` must never advance `arlp_stage_index` again. The CLI resume entry is documented in `references/standalone-cli.md`.
...
After a redfruit review, the browser opens `素材/文案列表查看`, clicks one material card to enter selection mode, then uses `全选 -> 全选所有` before `编辑 -> 增加ARLP`. Complete these configurations in order, with a separate all-material success gate and checkpoint after each stage:

1. `红果免费漫剧(8704)` + `番茄免费小说(1967)` on `广点通、头条内广、穿山甲联盟、union_app、粉丝通、内广-DPA、UC浏览器、sem`.
2. `短剧端原生IAA(796433)` on `头条内广`.
3. `番茄畅听(3040)` on `广点通、头条内广、穿山甲联盟、union_app、sem`.
...
- Exception only for stage 2 (`短剧端原生IAA(796433)` on `头条内广`): if the operation task says `修改ARLP / 已失败`, enter `素材管理`, search any one CID from this batch, click that material card body, scroll the material detail drawer to `审核信息`, and inspect the exact row `短剧端原生IAA(duanju1)-头条内广`. Only when this same row explicitly says `过审`, `审核通过`, `执行成功`, or `可以新建广告` may stage 2 be treated as successful. Do not inspect the failed operation task row's detail/tooltip for this rule. A success belonging to any other product/platform must not satisfy this fallback. Stage 1, stage 3, and Soda Music keep the normal task-count/retry rule.
...
After all three ARLP stages are fully successful, the browser returns to the same material list, refreshes it, and runs `点第一张素材 -> 全选所有 -> 编辑 -> 修改分类标签`. Before filling, scan the scrollable modal and wait only for the classification fields required by this batch. If the modal shows `暂无...` or any required field is absent, cancel it and reopen it with exponential backoff; do not require a fixed total number of fields. Then fill `番茄畅听素材类型 > 剪辑制作 > 常规剪辑`, `番茄畅听IOS/非IOS > 通用素材`, `有无logo > 无logo以及其他的产品信息`, `自动过审 > 自动过审`, `免费短剧-素材剪辑形式 > 原片剪辑`, `是否带免费利益点 > 是`, and `有无歌曲名露出(非音乐类素材不要打) > 非歌曲方向素材`. Set `是否为AI素材` to `非AI素材` for `纯短剧`, otherwise `AI素材`. Set `小程序系产品-内容体裁` to `短剧` for `纯短剧`, otherwise `漫剧`. Set `番茄畅听功能卖点` to the exact branch: `短剧 > 短剧内容 > 红果同步短剧 > 纯短剧内容`, `动态漫 > 漫剧内容 > 红果同步漫剧 > 漫剧剪辑`, or `仿真人短剧 > 仿真人内容 > 红果同步仿真人`. Each field is checkpointed after its leaf value is verified. If the browser stops before save, resume reopens the modal, rechecks completed field values, fills only missing/mismatched fields, then saves; a successful save records `save_status=success` and does not open an ARLP task-detail page.

Classification completion is the successful save on the material page; it must not call the ARLP `查看任务详情` helper or wait for another operation-task page. If the browser is interrupted while the classification checkpoint is still `classification_submitting`, resume returns to material management and repeats the idempotent full-selection save. The selector deliberately waits for `全选所有` and does not silently fall back to `全选当前页`.

## Redfruit Preflight

Redfruit runs a blocking preflight after the work-order search and before `新建创意单元`:

1. Read the order title from the search result row and normalize the drama/order category to `动态漫`, `仿真人`, or `纯短剧`; accept `真人剧` and `真人实拍短剧` as pure-short-drama aliases.
