...
当前 `HEAD` 必须包含最新基线，否则停止并要求先同步上游，避免将缺失的上游改动反向带入提交。新增已有 PR 的改动时还要求 PR 头包含该基线，当前 `HEAD` 包含 PR 头；差异比较和 worktree 改用同一个已抓取的 PR 头 SHA，从而追加提交并保留 PR 历史。PR 落后上游时，应先更新 PR 分支并同步本地，再提交增量。完全相同提交的 API 重试走下述独立验证路径，不要求当前 `HEAD` 已包含上次临时 worktree 推出的提交。
...
- 有远端分支但没有打开的 PR，或已有 PR 但当前 HEAD 尚不包含上次推送的提交：仅当远端提交恰有一个父提交，该父提交包含最新基线且属于当前 HEAD 历史，完整文件树又与通过检查的待提交树完全一致时，只重试 PR API，不 commit、不 push；否则停止，避免覆盖未知改动。

这允许“push 成功、PR API 随后失败”的相同提交安全重试，覆盖 PR 未创建、PR 已创建但响应丢失、已有 PR 更新响应丢失三种情况。后续增量仍需先同步 PR 头。所有推送都不带 force 选项，远端并发变化导致非快进时由 Git 拒绝。
...
远端参数只接受安全的简单名称，拒绝前导 `-`、路径和 URL；基线仓库在 fetch 前由配置的 remote URL 解析和校验。可自动创建的推送 remote 仍须使用安全名称，不能将 CLI 参数解释为 Git 命令选项。
