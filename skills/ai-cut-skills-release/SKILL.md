...
5. 执行 Skill 结构校验、仓库清单校验、语法检查，以及仓库 `tests/` 和每个所选 Skill 的 `tests/` 下的 unittest。
...
多个 Skill 或测试文件需要显式重复 `--skill` / `--include`。`--target-repository` 只能显式指定与 `upstream` 相同的仓库，不能跨仓库写入。没有目标变更、校验失败、GitHub 账户不匹配、同名远端分支不满足安全复用条件，或 PR 已产生不可安全判断的冲突时停止并报告，不覆盖现有工作。
...
默认只读预检在可清理的 detached worktree 内运行；只有 `--execute` 才提交和推送。已有受管 PR 的更新不使用任何 force 选项，并发远端更新导致非快进时停止。这个 Skill 只创建或更新 PR，不自动审批或合并。
